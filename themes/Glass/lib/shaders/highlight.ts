 ...
 * glow ~50x dimmer (e.g. 0.15^2 = 0.0225 -- essentially invisible).
 * ------------------------------------------------------------------ */
...
void main() {
    ...
    // Gaussian convolution of the hard stroke mask -- 3-tap (sigma-spaced).
    // The original's BlurMaskFilter has sigma = blurRadius = 0.25dp -> 0.25px at
    // dpr=1. At this sub-pixel sigma, only 3 taps (at -sigma, 0, +sigma) are needed
    // -- the Gaussian weight at +/-2sigma is exp(-2) ~= 0.14, negligible. This
    ...
    // cycles, a 20x reduction with identical visual result at sigma=0.25.
    //   hardMask(sd) = 1.0 if |sd| < strokeHalf, else 0.0
    //   blurred(sd) = sigma hardMask(sd - offset_k) * gauss(offset_k, sigma)
    ...
    float wSum = 0.0;
    for (int i = -1; i <= 1; i++) {
        float offset = float(i) * sigma;  // taps at -sigma, 0, +sigma
 ...
 *
 * Composite: strokeMask x intensity x color.
 * Blend modes (set by the renderer):
...
void main() {
    ...
    // The stroke mask is drawn in original space (origSizeX x origSizeY + margin).
