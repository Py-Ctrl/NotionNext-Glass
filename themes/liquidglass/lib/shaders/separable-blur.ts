 ...
 *
 * A 2D Gaussian kernel G(x,y) = G(x)*G(y) is separable: an NxN 2D
 * convolution (N^2 samples) can be computed as two 1D convolutions
 ...
 *  Faithful to Skia's convertRadiusToSigma: sigma = radius * 0.57735 + 0.5.
 *  Then N = 2*ceil(3sigma)+1 (3-sigma cutoff), capped at 33 (shader max).
 *  Small radii -> small sigma -> few taps. E.g. radius=2px -> sigma~=1.65 -> N=11.
 *  radius=8px -> sigma~=5.12 -> N=33. */
 ...
 * sigma), where the `radius` param IS the Gaussian sigma (not 2sigma/3sigma).
 *
 ...
 *      Skia's convertRadiusToSigma radius. offset = tap_position (in sigma
 ...
 *
 * Tap count: N = 2*ceil(3sigma)+1, minimum 3 (sigma>0). For sigma=0.25px -> N=3.
 * The 3-tap kernel (offsets -1, 0, +1 in sigma units) samples at -sigma, 0, +sigma
 * pixels -- a minimal but faithful Gaussian.
 * ------------------------------------------------------------------ */

/** Generate a Gaussian kernel for the highlight blur.
 *  taps are at integer multiples of sigma from -half..+half.
 *  Returns offsets (in sigma units) + normalized weights. */
function generateHighlightBlurKernel1D(tapCount: number): Array<{ offset: number; weight: number }> {
  ...
  let totalW = 0
  for (let i = 0; i < tapCount; i++) {
    // Integer-spaced offsets in sigma units: -half, ..., 0, ..., +half
    const offset = i - half
    const w = Math.exp(-0.5 * offset * offset)
    taps.push({ offset, weight: w })
    totalW += w
...
void main() {
    ...
    // pxToUv converts a pixel offset to a UV offset. offset (in sigma units) *
 ...
 *  N = 2*ceil(3sigma)+1, min 3 (for sigma>0), max 33 (shader const loop bound).
 *  sigma=0.25 -> N=3; sigma=1 -> N=7; sigma=5 -> N=33. */
