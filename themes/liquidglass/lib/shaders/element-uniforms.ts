...
// The indicator's backdrop = CombinedBackdrop(wallpaper, innerBackdrop) where
// innerBackdrop (tabsBackdrop) is a hidden Row with ColorFilter.tint(accentColor). Only the
...
// --- inner backdrop plate rim highlight stroke mask (Canvas2D, same approach as outer rim) ---
...
// The mask is drawn for the inner backdrop plate capsule shape (uContainerRect dimensions)
